...
const proposalService = {
  async submitProposal(formElement) {
    ...
    try {
      ...
      loadingOverlay.innerHTML = "<p>Sending proposal...</p>";
  ...
  showModal(responseText, requestId) {
      ...
      <div class="custom-modal-overlay">
        <div class="custom-modal">
          ...
          <div id="rating-popup" class="rating-popup hidden">
            ...
            <div class="stars">
              ${[1, 2, 3, 4, 5]
                .map((num) => `<span class="star" data-rating="${num}">★</span>`)
                .join("")}
            </div>
    ...
    document.getElementById("submit-rating").addEventListener("click", () => {
      const rating = parseInt(document.getElementById("rating-popup").dataset.rating) || 0;
      if (rating === 0) {
  ...
  submitRate(requestId, rating) {
    fetch(`http://localhost:8080/apollo-proposals/api/requests/${requestId}/${rating}`, {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
      },
